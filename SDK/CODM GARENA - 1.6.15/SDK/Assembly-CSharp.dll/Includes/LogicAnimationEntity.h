#pragma once
#include <Il2Cpp/Il2Cpp.h>

class LogicAnimationEntity
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "LogicAnimationEntity"));
	}

	template <typename T = int32_t> T& StateNameID() {
		return *(T*)((uintptr_t)this + 0x8);
	}
	template <typename T = int32_t> T& Layer() {
		return *(T*)((uintptr_t)this + 0xC);
	}
	template <typename T = float> T& Time() {
		return *(T*)((uintptr_t)this + 0x10);
	}
	template <typename T = bool> T& IsFinished() {
		return *(T*)((uintptr_t)this + 0x14);
	}
	template <typename T = uintptr_t> T& m_Owner() {
		return *(T*)((uintptr_t)this + 0x18);
	}
	template <typename T = uintptr_t> T& m_AnimationComponent() {
		return *(T*)((uintptr_t)this + 0x1C);
	}
	template <typename T = float> T& m_TickTime() {
		return *(T*)((uintptr_t)this + 0x20);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_Play() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_Tick() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}

	template <typename T = void> T Play() {
		return ((T (*)(LogicAnimationEntity*))(Il2CppBase() + 0x19AC8D8))(this);
	}
	template <typename T = void> T Tick(float deltaTime) {
		return ((T (*)(LogicAnimationEntity*, float))(Il2CppBase() + 0x19AC2DC))(this, deltaTime);
	}

};

}
