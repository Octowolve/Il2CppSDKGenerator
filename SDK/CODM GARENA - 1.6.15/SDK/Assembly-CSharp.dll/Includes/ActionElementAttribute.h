#pragma once
#include <Il2Cpp/Il2Cpp.h>

class ActionElementAttribute
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "ActionElementAttribute"));
	}

	template <typename T = Il2CppString*> T& title() {
		return *(T*)((uintptr_t)this + 0x8);
	}
	template <typename T = Il2CppString*> T& info() {
		return *(T*)((uintptr_t)this + 0xC);
	}
	template <typename T = bool> T& bClamp() {
		return *(T*)((uintptr_t)this + 0x10);
	}
	template <typename T = float> T& Min() {
		return *(T*)((uintptr_t)this + 0x14);
	}
	template <typename T = float> T& Max() {
		return *(T*)((uintptr_t)this + 0x18);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_getElementInfo() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}

	template <typename T = Il2CppArray<uintptr_t>*> static T getElementInfo(uintptr_t inType) {
		return ((T (*)(void *, uintptr_t))(Il2CppBase() + 0x4A12EB0))(0, inType);
	}

};

}
