#pragma once
#include <Il2Cpp/Il2Cpp.h>
namespace WellFired {

class USChangeColor
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "WellFired", "USChangeColor"));
	}

	template <typename T = uintptr_t> T& newColor() {
		return *(T*)((uintptr_t)this + 0x1C);
	}
	template <typename T = uintptr_t> T& previousColor() {
		return *(T*)((uintptr_t)this + 0x2C);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_FireEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_ProcessEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_StopEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0xC);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_UndoEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x10);
	}

	template <typename T = void> T FireEvent() {
		return ((T (*)(USChangeColor*))(Il2CppBase() + 0x4A3903C))(this);
	}
	template <typename T = void> T ProcessEvent(float deltaTime) {
		return ((T (*)(USChangeColor*, float))(Il2CppBase() + 0x4A392F0))(this, deltaTime);
	}
	template <typename T = void> T StopEvent() {
		return ((T (*)(USChangeColor*))(Il2CppBase() + 0x4A39390))(this);
	}
	template <typename T = void> T UndoEvent() {
		return ((T (*)(USChangeColor*))(Il2CppBase() + 0x4A39440))(this);
	}
	template <typename T = void> T xLuaBaseProxy_StopEvent() {
		return ((T (*)(USChangeColor*))(Il2CppBase() + 0x4A39628))(this);
	}
	template <typename T = void> T xLuaBaseProxy_UndoEvent() {
		return ((T (*)(USChangeColor*))(Il2CppBase() + 0x4A39630))(this);
	}

};

}
