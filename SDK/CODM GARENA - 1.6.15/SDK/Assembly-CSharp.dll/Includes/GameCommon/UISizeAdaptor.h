#pragma once
#include <Il2Cpp/Il2Cpp.h>
namespace GameCommon {

class UISizeAdaptor
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "GameCommon", "UISizeAdaptor"));
	}

	template <typename T = uintptr_t> T& mContainer() {
		return *(T*)((uintptr_t)this + 0xC);
	}
	template <typename T = int32_t> T& mWidth() {
		return *(T*)((uintptr_t)this + 0x10);
	}
	template <typename T = int32_t> T& mHeight() {
		return *(T*)((uintptr_t)this + 0x14);
	}
	template <typename T = int32_t> T& originWidth() {
		return *(T*)((uintptr_t)this + 0x18);
	}
	template <typename T = int32_t> T& originHeight() {
		return *(T*)((uintptr_t)this + 0x1C);
	}
	template <typename T = bool> T& mIsFillScreen() {
		return *(T*)((uintptr_t)this + 0x20);
	}
	template <typename T = int32_t> T& LeftOffset() {
		return *(T*)((uintptr_t)this + 0x24);
	}
	template <typename T = int32_t> T& RightOffset() {
		return *(T*)((uintptr_t)this + 0x28);
	}
	template <typename T = int32_t> T& TopOffset() {
		return *(T*)((uintptr_t)this + 0x2C);
	}
	template <typename T = int32_t> T& BottomOffset() {
		return *(T*)((uintptr_t)this + 0x30);
	}
	template <typename T = int32_t> T& mProtected() {
		return *(T*)((uintptr_t)this + 0x34);
	}
	template <typename T = bool> T& mFixHeight() {
		return *(T*)((uintptr_t)this + 0x38);
	}
	template <typename T = bool> T& mFixWidth() {
		return *(T*)((uintptr_t)this + 0x39);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_Awake() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_OnEnable() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_OnApplicationPause() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0xC);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_Resize() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x10);
	}

	template <typename T = void> T Awake() {
		return ((T (*)(UISizeAdaptor*))(Il2CppBase() + 0x49FE2E0))(this);
	}
	template <typename T = void> T OnEnable() {
		return ((T (*)(UISizeAdaptor*))(Il2CppBase() + 0x49FF0FC))(this);
	}
	template <typename T = void> T OnApplicationPause(bool pause) {
		return ((T (*)(UISizeAdaptor*, bool))(Il2CppBase() + 0x49FF1A0))(this, pause);
	}
	template <typename T = void> T Resize() {
		return ((T (*)(UISizeAdaptor*))(Il2CppBase() + 0x49FE384))(this);
	}

};

}
