#pragma once
#include <Il2Cpp/Il2Cpp.h>
namespace WellFired {

class USRotateObjectEvent
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "WellFired", "USRotateObjectEvent"));
	}

	template <typename T = float> T& rotateSpeedPerSecond() {
		return *(T*)((uintptr_t)this + 0x1C);
	}
	template <typename T = Il2CppVector3> T& rotationAxis() {
		return *(T*)((uintptr_t)this + 0x20);
	}
	template <typename T = Il2CppQuaternion> T& sourceOrientation() {
		return *(T*)((uintptr_t)this + 0x2C);
	}
	template <typename T = Il2CppQuaternion> T& previousRotation() {
		return *(T*)((uintptr_t)this + 0x3C);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_FireEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_ProcessEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_StopEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0xC);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_UndoEvent() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x10);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_Update() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x14);
	}

	template <typename T = void> T FireEvent() {
		return ((T (*)(USRotateObjectEvent*))(Il2CppBase() + 0x4A448A8))(this);
	}
	template <typename T = void> T ProcessEvent(float deltaTime) {
		return ((T (*)(USRotateObjectEvent*, float))(Il2CppBase() + 0x4A449E8))(this, deltaTime);
	}
	template <typename T = void> T StopEvent() {
		return ((T (*)(USRotateObjectEvent*))(Il2CppBase() + 0x4A44B34))(this);
	}
	template <typename T = void> T UndoEvent() {
		return ((T (*)(USRotateObjectEvent*))(Il2CppBase() + 0x4A44BE4))(this);
	}
	template <typename T = void> T Update() {
		return ((T (*)(USRotateObjectEvent*))(Il2CppBase() + 0x4A44D34))(this);
	}
	template <typename T = void> T xLuaBaseProxy_StopEvent() {
		return ((T (*)(USRotateObjectEvent*))(Il2CppBase() + 0x4A44E00))(this);
	}
	template <typename T = void> T xLuaBaseProxy_UndoEvent() {
		return ((T (*)(USRotateObjectEvent*))(Il2CppBase() + 0x4A44E08))(this);
	}

};

}
