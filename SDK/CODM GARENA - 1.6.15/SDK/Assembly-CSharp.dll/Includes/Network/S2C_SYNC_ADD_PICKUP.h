#pragma once
#include <Il2Cpp/Il2Cpp.h>
namespace Network {

class S2CSYNCADDPICKUP
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "Network", "S2C_SYNC_ADD_PICKUP"));
	}

	template <typename T = int32_t> T& ActorId() {
		return *(T*)((uintptr_t)this + 0xC);
	}
	template <typename T = int32_t> T& PickupItemId() {
		return *(T*)((uintptr_t)this + 0x10);
	}
	template <typename T = int32_t> T& PostionId() {
		return *(T*)((uintptr_t)this + 0x14);
	}
	template <typename T = unsigned char> T& Type() {
		return *(T*)((uintptr_t)this + 0x18);
	}
	template <typename T = uintptr_t> T& Pos() {
		return *(T*)((uintptr_t)this + 0x1C);
	}
	template <typename T = int16_t> T& Duration() {
		return *(T*)((uintptr_t)this + 0x20);
	}
	template <typename T = int32_t> T& ClipNum() {
		return *(T*)((uintptr_t)this + 0x24);
	}
	template <typename T = int32_t> T& nAmmoInMagzine() {
		return *(T*)((uintptr_t)this + 0x28);
	}
	template <typename T = unsigned char> T& PickUpItemAttachmentCount() {
		return *(T*)((uintptr_t)this + 0x2C);
	}
	template <typename T = Il2CppArray<uintptr_t>*> T& PickUpItemAttachments() {
		return *(T*)((uintptr_t)this + 0x30);
	}
	template <typename T = unsigned char> T& PickUpItemSkillCount() {
		return *(T*)((uintptr_t)this + 0x34);
	}
	template <typename T = Il2CppArray<uintptr_t>*> T& PickUpItemSkills() {
		return *(T*)((uintptr_t)this + 0x38);
	}
	template <typename T = uint32_t> T& CamoID() {
		return *(T*)((uintptr_t)this + 0x3C);
	}
	template <typename T = uint32_t> T& OriginOwnerID() {
		return *(T*)((uintptr_t)this + 0x40);
	}
	template <typename T = int16_t> T& ItemNum() {
		return *(T*)((uintptr_t)this + 0x44);
	}
	template <typename T = int32_t> T& Param1() {
		return *(T*)((uintptr_t)this + 0x48);
	}
	template <typename T = unsigned char> T& IsInBox() {
		return *(T*)((uintptr_t)this + 0x4C);
	}
	template <typename T = uintptr_t> T& FromPos() {
		return *(T*)((uintptr_t)this + 0x50);
	}
	template <typename T = uintptr_t> T& Orient() {
		return *(T*)((uintptr_t)this + 0x54);
	}
	template <typename T = uint32_t> T& BoxActorId() {
		return *(T*)((uintptr_t)this + 0x58);
	}
	template <typename T = uint16_t> T& Radius() {
		return *(T*)((uintptr_t)this + 0x5C);
	}
	template <typename T = unsigned char> T& Camp() {
		return *(T*)((uintptr_t)this + 0x5E);
	}
	template <typename T = uint32_t> T& PapId() {
		return *(T*)((uintptr_t)this + 0x60);
	}
	template <typename T = uintptr_t> T& Decorations() {
		return *(T*)((uintptr_t)this + 0x64);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_Unserialize() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_GetSize() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}

	template <typename T = void> T Unserialize(Il2CppArray<uintptr_t>* bytes, uintptr_t position) {
		return ((T (*)(S2CSYNCADDPICKUP*, Il2CppArray<uintptr_t>*, uintptr_t))(Il2CppBase() + 0x4E6298C))(this, bytes, position);
	}
	template <typename T = int32_t> static T GetSize(Il2CppArray<uintptr_t>* data, int32_t DataIndex) {
		return ((T (*)(void *, Il2CppArray<uintptr_t>*, int32_t))(Il2CppBase() + 0x4E62F4C))(0, data, DataIndex);
	}
	template <typename T = void> T xLuaBaseProxy_Unserialize(Il2CppArray<uintptr_t>* P0, uintptr_t P1) {
		return ((T (*)(S2CSYNCADDPICKUP*, Il2CppArray<uintptr_t>*, uintptr_t))(Il2CppBase() + 0x4E630D4))(this, P0, P1);
	}

};

}
