#pragma once
#include <Il2Cpp/Il2Cpp.h>
namespace GameBase {

class WeaponVtolWeapon
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "GameBase", "WeaponVtolWeapon"));
	}

	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_IsFireButtonEnabled() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_PreBeginState() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_PrepareInputParam() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0xC);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_GetFireInterval() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x10);
	}

	template <typename T = bool> T IsFireButtonEnabled() {
		return ((T (*)(WeaponVtolWeapon*))(Il2CppBase() + 0x458D8E4))(this);
	}
	template <typename T = void> T PreBeginState(uintptr_t newState, uintptr_t oldState) {
		return ((T (*)(WeaponVtolWeapon*, uintptr_t, uintptr_t))(Il2CppBase() + 0x458D984))(this, newState, oldState);
	}
	template <typename T = void> T PrepareInputParam(uintptr_t param) {
		return ((T (*)(WeaponVtolWeapon*, uintptr_t))(Il2CppBase() + 0x458DA94))(this, param);
	}
	template <typename T = float> T GetFireInterval() {
		return ((T (*)(WeaponVtolWeapon*))(Il2CppBase() + 0x458DB58))(this);
	}
	template <typename T = bool> T xLuaBaseProxy_IsFireButtonEnabled() {
		return ((T (*)(WeaponVtolWeapon*))(Il2CppBase() + 0x458DD2C))(this);
	}
	template <typename T = void> T xLuaBaseProxy_PreBeginState(uintptr_t P0, uintptr_t P1) {
		return ((T (*)(WeaponVtolWeapon*, uintptr_t, uintptr_t))(Il2CppBase() + 0x458DD34))(this, P0, P1);
	}
	template <typename T = void> T xLuaBaseProxy_PrepareInputParam(uintptr_t P0) {
		return ((T (*)(WeaponVtolWeapon*, uintptr_t))(Il2CppBase() + 0x458DD3C))(this, P0);
	}
	template <typename T = float> T xLuaBaseProxy_GetFireInterval() {
		return ((T (*)(WeaponVtolWeapon*))(Il2CppBase() + 0x458DD44))(this);
	}

};

}
