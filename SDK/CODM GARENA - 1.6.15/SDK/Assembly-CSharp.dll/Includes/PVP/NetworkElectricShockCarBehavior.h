#pragma once
#include <Il2Cpp/Il2Cpp.h>
namespace PVP {

class NetworkElectricShockCarBehavior
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "PVP", "NetworkElectricShockCarBehavior"));
	}

	template <typename T = uintptr_t> T& onStart() {
		return *(T*)((uintptr_t)this + 0x28);
	}
	template <typename T = uintptr_t> T& onStop() {
		return *(T*)((uintptr_t)this + 0x2C);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix0_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x0);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix1_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x4);
	}
	template <typename T = uintptr_t> static T& _c__Hotfix2_ctor() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x8);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_BehaviorMain() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0xC);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_OnUpdateRepInfo() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x10);
	}
	template <typename T = uintptr_t> static T& __Hotfix0_OnStop() {
		return *(T*)((uintptr_t)StaticClass()->static_fields + 0x14);
	}

	template <typename T = bool> T get_UseCoroutine() {
		return ((T (*)(NetworkElectricShockCarBehavior*))(Il2CppBase() + 0x348A508))(this);
	}
	template <typename T = uintptr_t> T BehaviorMain() {
		return ((T (*)(NetworkElectricShockCarBehavior*))(Il2CppBase() + 0x348A510))(this);
	}
	template <typename T = void> T OnUpdateRepInfo(uintptr_t skillRepInfo) {
		return ((T (*)(NetworkElectricShockCarBehavior*, uintptr_t))(Il2CppBase() + 0x348A37C))(this, skillRepInfo);
	}
	template <typename T = void> T OnStop() {
		return ((T (*)(NetworkElectricShockCarBehavior*))(Il2CppBase() + 0x3489B50))(this);
	}
	template <typename T = uintptr_t> T xLuaBaseProxy_BehaviorMain() {
		return ((T (*)(NetworkElectricShockCarBehavior*))(Il2CppBase() + 0x348A5F8))(this);
	}
	template <typename T = void> T xLuaBaseProxy_OnUpdateRepInfo(uintptr_t P0) {
		return ((T (*)(NetworkElectricShockCarBehavior*, uintptr_t))(Il2CppBase() + 0x348A600))(this, P0);
	}
	template <typename T = void> T xLuaBaseProxy_OnStop() {
		return ((T (*)(NetworkElectricShockCarBehavior*))(Il2CppBase() + 0x348A608))(this);
	}

};

}
